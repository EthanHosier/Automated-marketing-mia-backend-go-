...
func ThemePrompt(pageContents []types.BodyContentsScrapeResponse, businessSummary types.StoredBusinessSummary, targetAudience string, additionalInstructions string, backlink string, imageDescriptions []string) string {
	for _, pageContent := range pageContents {
		pageContent.ImageUrls = []string{}
	}

	ta := targetAudiencePrompt(targetAudience, businessSummary.TargetAudience)

	return fmt.Sprintf(themeGeneration, businessSummary, pageContents, ta, additionalInstructions, imageDescriptions)
}
